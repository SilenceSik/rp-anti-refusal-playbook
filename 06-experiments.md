...
## 关键机制分析：Gemini 深度思考（reasoning）导致截断风险翻倍

**诱因**：Gemini 开启深度思考时，`reasoning_content` 思考链包含了**整套双 Safe 包裹 + 完整角色正文的「草稿版」**，而 `content` 字段是「正式版」。两版正文措辞高度相似但不完全一致，说明模型独立生成了两次。
...
**对策**：
- 在配置中统一关闭 Gemini 模型的 `reasoning: false`
